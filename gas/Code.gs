...
function generateReply_(message, history) {
  ...
  if (!apiKey) throw new Error('尚未在 Apps Script 指令碼屬性設定 OPENAI_API_KEY。');
  ...
  if (statusCode < 200 || statusCode >= 300) throw new Error('OpenAI API 呼叫失敗：' + rawText);

  const result = JSON.parse(rawText);
  const reply = extractTextFromResponse_(result);
  if (!reply) throw new Error('OpenAI API 未回傳可顯示的文字內容。');
  return reply;
}
...
function buildInputText_(message, history) {
  ...
  return ['以下是最近的聊天紀錄，請自然延續對話。', lines.join('\n')].join('\n\n');
}
...
function extractTextFromResponse_(result) {
  ...
  for (let i = 0; i < output.length; i += 1) {
    ...
    for (let j = 0; j < content.length; j += 1) {
      if (content[j].type === 'output_text' && content[j].text) return String(content[j].text).trim();
    }
  }
  return '';
}
...
function appendLogRow_(timestamp, speaker, message) {
  getOrCreateSheet_().appendRow([timestamp, speaker, message, OPENAI_MODEL]);
}
